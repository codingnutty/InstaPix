class InstapostWorker
  include Sidekiq::Worker
  sidekiq_options :retry => false

  def perform(next_page_url, collection_id, tag, start_date, end_date)
    paginate(next_page_url, collection_id, tag, start_date, end_date)
  end

  def contains_hashtag?(text, tag)
    text.downcase.include?(tag)
  end
  ...
  def is_a_video?(result)
   ...
   def filter_data(result, tag, start_date, end_date)
    ...
    if caption_empty?(result)
      ...
      if contains_hashtag?(filtered_data[:description], tag)
        if falls_within_date_range?(created_time, start_date, end_date)
        ...
        comments.each do |comment|
          ...
          if contains_hashtag?(comment["text"], tag)
            p comment["text"]
  ...
  def paginate(next_page_url, search_id, tag, start_date, end_date)
    response = HTTParty.get(next_page_url)

    response_body = JSON.parse(response.body)

      response_body["data"].each do |result|
        filtered_result_data = filter_data(result, tag, start_date, end_date)
        @result = Result.create(ig_username: filtered_result_data[:ig_username], content_type: filtered_result_data[:content_type], ig_link: filtered_result_data[:ig_link], image_url: filtered_result_data[:image_url], video_url: filtered_result_data[:video_url], description: filtered_result_data[:description], collection_id: collection_id)
      end

      if has_next_page_link?(response_body)
        paginate(response_body["pagination"]["next_url"], collection_id, tag, start_date, end_date)
      else
        return
      end
  end
